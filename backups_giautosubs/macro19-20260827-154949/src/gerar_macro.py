...
# Onde o botao "Display Config" grava o dump. Arquivo proprio, e reescrito a cada
# clique: e' pra ser aberto e copiado inteiro, nao para virar historico.
LOG_CONFIG = r"D:\CanalYtbe\BatataQuente\pre_production\giautosubs\_giconfig.txt"
...
__LOG_LUA__
	...
	--
	-- O elemento 1 fica fora do `CONTROLE_DA_CAMADA` de proposito: a cor BASE
	-- dele e' a do falante, escrita clipe a clipe, e um seletor unico apagaria
	-- isso em todas as legendas. Aqui ele entra so' pela cor ATIVA - o `WordFill`
	-- pinta a palavra que esta sendo dita, e a cor de cada pessoa continua
	-- valendo em todas as outras.
	...
	local function cor_de(c, qual)
		local base = c[qual]
		local de = c.controle
		if qual == "cor_ativa" and c.elemento == 1 and palavra_colorida then
			de = "WordFill"
		end
		if de then
			return {
	...
	-- onde o destaque e' so' a bolha). Sem uma camada, o controle nao teria onde
	-- agir - entao ela nasce aqui, com a cor BASE lida do proprio Text+: e' la
	-- que a cor do falante deste clipe ja esta.
	if palavra_colorida then
		...
		if not tem then
			local function base(nome, padrao)
				local v
				pcall(function() v = template:GetInput(nome) end)
				if v == nil then return padrao end
				return v
			end
			camadas[#camadas + 1] = {
				elemento = 1, on_base = 1, on_ativo = 1,
				cor_base = { base("Red1", 1), base("Green1", 1), base("Blue1", 1) },
			}
		end
	end
...
__LOG_LUA__
	...
	-- Editar o texto a mao MUDA o numero de caracteres, e o destaque por palavra
	-- endereca o texto por indice de caractere (`GiWordTiming`). Realinhar
	-- palavra a palavra so' o script sabe fazer - o que da' pra fazer aqui e'
	-- dizer, em vez de deixar o destaque errar em silencio.
	local tempos = template:GetData("GiWordTiming")
	local ultimoIndice = 0
	for _, t in ipairs(tempos or {}) do
		if (t.endIndex or 0) > ultimoIndice then ultimoIndice = t.endIndex end
	end
	local _, letras = novo:gsub("[^\\128-\\191]", "")
	if ultimoIndice > 0 and letras < ultimoIndice + 1 then
		diga(string.format("[GiAutoSubs] WARNING: the text is now %d characters "
			.. "but the per-word highlight was built for %d - run the script "
			.. "again to realign it", letras, ultimoIndice + 1))
	end
...
# O corpo do botao "Display Config".
#
# Imprime o estilo do clipe no Console num formato que da' pra copiar e colar -
# e que volta pra ca' como um estilo do `estilos.json` sem ninguem traduzir nada
# no meio. E' a resposta pra "essa e' a minha configuracao favorita, guarda ela".
#
...
#
# Junto vem o que um estilo NAO carrega mas explica um "nao funcionou": a versao
# do macro dentro DESTE clipe, e se o texto original esta guardado.
_MOSTRAR_CONFIG = """
local function gi_dono(comp, tool, nome)
	local v
	pcall(function() v = tool and tool:GetData(nome) end)
...
local macro = gi_dono(comp, tool, "InputKeys")
local template = comp:FindTool("Template") or tool

local linhas = {}
local function diz(s)
	linhas[#linhas + 1] = s
	print(s)
end

diz("[GiAutoSubs] ===== config of this caption =====")

local versao = "?"
pcall(function() versao = tostring(template:GetInput("GiAutoSubsVersao")) end)
diz("[GiAutoSubs] macro version in this clip: " .. versao)

local original
pcall(function() original = template:GetData("GiTextOriginal") end)
diz("[GiAutoSubs] text as typed: " .. tostring(original or "(not stored yet)"))

-- Le do MESMO jeito que o script: `GetInputValues` percorre o `InputKeys`.
local chaves, valores = {}, {}
do
	local f = macro and macro:GetData("GetInputValues")
	if f then
		local ok, r = pcall(function() return loadstring(f)()(macro) end)
		if ok then valores = r or {} end
	end
	local ik = macro and macro:GetData("InputKeys") or {}
	for _, k in ipairs(ik) do chaves[#chaves + 1] = k end
end

if #chaves == 0 then
	diz("[GiAutoSubs] could not read InputKeys from the macro - this clip "
		.. "carries an old copy (see the version above)")
else
	table.sort(chaves)
	for _, k in ipairs(chaves) do
		local v = valores[k]
		if v == nil then pcall(function() v = template:GetInput(k) end) end
		if type(v) == "number" and v == math.floor(v) then
			v = string.format("%d", v)
		elseif type(v) == "number" then
			v = string.format("%.4f", v)
		elseif type(v) == "table" then
			-- Position e Offset sao pares. `tostring` numa tabela imprime o
			-- ENDERECO ("table: 0x01e4b..."), que nao serve pra nada de volta.
			local partes = {}
			for i = 1, 4 do
				if v[i] == nil then break end
				partes[#partes + 1] = string.format("%.4f", v[i])
			end
			v = "{ " .. table.concat(partes, ", ") .. " }"
		end
		diz(string.format("[GiAutoSubs]   %-26s %s", k, tostring(v)))
	end
	diz("[GiAutoSubs] ===== " .. #chaves .. " controls =====")
end

-- Tambem em arquivo: o Console do Resolve nao deixa copiar log longo - ele rola
-- pra fora antes de dar pra selecionar, e sao ~60 linhas.
pcall(function()
	local fh = io.open("__CONFIG__", "w")
	if not fh then return end
	fh:write(table.concat(linhas, "\\n"), "\\n")
	fh:close()
	print("[GiAutoSubs] saved to __CONFIG__")
end)
"""
...
# ATENCAO: a ORDEM destas quatro strings e' o valor gravado em `Level{n}`, e
# ela esta errada desde sempre - ver ESTADO.md secao 8. O dump diz que a lista
# real do Text+ e' [Text, Line, Word, Character], entao o que aqui se chama
# "Character" desenha por LINHA e vice-versa. "Word" acerta por coincidencia, e
# e' o unico que o estilo usa - por isso nunca apareceu.
#
# Traduzido na MESMA ordem de proposito: consertar isso exige mexer no `NIVEIS`
# do giautosubs.py junto, senao o Inspector e o script passam a discordar.
_NIVEIS = ("Text+ default", "Character", "Word", "Line")
...
def controles():
    ...
    partes = [
        ...
        _combo("TextBoxLevel", "Level", _NIVEIS, 3),
...
def controles_fim():
    ...
    return "\n".join([
        ...
        _botao("DisplayConfig", "Display Config",
               _MOSTRAR_CONFIG.replace("__CONFIG__",
                                       LOG_CONFIG.replace("\\", "\\\\"))),
    ])
...
def controles_topo():
    ...
    """
    return "\n".join([
        _rotulo("ApplyStyleLabel", "Actions", 1),
...
# pra restaurar.
_FORA_DO_PRESET = {"GiAutoSubsVersao", "ApplyStyle", "DisplayConfig"}
...
def _instance_inputs_apos(texto, novo, fonte, contador):
    """Poe InstanceInputs logo depois do InstanceInput cujo Source e' `fonte`.

    E' o que permite entrar no MEIO de um grupo do AutoSubs (o `Case` fica entre
    `Style` e `Size`) em vez de so' antes ou depois dele.
    """
    m = _INPUTS_MACRO.search(texto)
    if not m:
        contador.append("WARNING: could not find the macro Inputs block")
    ...
    ind, corpo = m.group(1), m.group(2)
    alvo = re.search(
        r"(?ms)^\t+\w+ = InstanceInput \{(?:(?!InstanceInput).)*?"
        r"^\t+Source = \"" + re.escape(fonte) + r"\",.*?^\t+\},\n?", corpo)
    if not alvo:
        contador.append(f"WARNING: '{fonte}' not found, {novo.strip()[:20]}... "
                        f"went to the end")
        return _anexar_instance_inputs(texto, novo, contador)
    corpo = (corpo[:alvo.end()].rstrip("\n") + "\n" + novo.rstrip("\n") + "\n"
             + corpo[alvo.end():])
    contador.append(f"the Case control sits right below '{fonte}' in the Text group")
    return (texto[:m.start()] + ind + "Inputs = ordered() {\n" + corpo
            + ind + "}," + texto[m.end():])


def _instance_inputs_apos_grupo(texto, novo, rotulo, contador):
    """Poe InstanceInputs logo DEPOIS do grupo aberto por `rotulo`.

    O Inspector do clipe segue a ordem deste bloco, entao "abaixo do grupo Text"
    e' literalmente "depois do ultimo controle do grupo Text" - ou seja, imediatamente
    antes do proximo `LabelControl`, que e' quem abre o grupo seguinte.
    """
    m = _INPUTS_MACRO.search(texto)
    if not m:
        contador.append("WARNING: could not find the macro Inputs block")
    ...
    blocos = list(re.finditer(
        r"(?ms)^\t+\w+ = InstanceInput \{.*?^\t+Source = \"(\w+)\",.*?^\t+\},\n?",
        corpo))

    inicio = next((i for i, b in enumerate(blocos) if b.group(1) == rotulo), None)
    if inicio is None:
        contador.append(f"WARNING: group '{rotulo}' not found, "
                        f"the Actions group went to the end")
        return _anexar_instance_inputs(texto, novo, contador)

    # o proximo rotulo ABRE o grupo seguinte, entao o corte e' onde ele comeca -
    # nao onde ele termina, que ja seria dentro do outro grupo
    fim = next((b.start() for b in blocos[inicio + 1:] if b.group(1) in rotulos),
               len(corpo))
    corpo = corpo[:fim].rstrip("\n") + "\n" + novo.rstrip("\n") + "\n" + corpo[fim:]
    contador.append(f"the Actions group sits right below '{rotulo}' in the Inspector")
    return (texto[:m.start()] + ind + "Inputs = ordered() {\n" + corpo
            + ind + "}," + texto[m.end():])
...
def instance_inputs(lista=None):
...
def aplicar(texto, defaults=None, inputs=None):
    ...
    # 8) InstanceInputs novos - os de estilo no fim do bloco, o grupo Actions
    #    logo abaixo do grupo "Text" (a ordem do bloco e' a ordem do Inspector, e
    #    o botao que fecha todo ajuste nao pode ficar depois de 40 controles e
    #    uma rolagem)
    texto = _anexar_instance_inputs(texto, _ind(instance_inputs(), 4), mudancas)
    texto = _instance_inputs_apos_grupo(texto, _ind(instance_inputs(NOVOS_TOPO), 4),
                                        "SubtitleLabel", mudancas)
    texto = _instance_inputs_apos(texto, _ind(instance_inputs(NOVOS_TEXTO), 4),
                                  "Style", mudancas)
    mudancas.append("InstanceInputs (the new controls showing up on the clip)")
