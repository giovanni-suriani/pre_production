...
for _, par in ipairs({ { "WordFillLabel", 4 }, { "BubbleLabel", 12 },
	{ "TextBoxLabel", 9 }, { "BoxShadowLabel", 9 } }) do
	local l = uc[par[1]]
...
--
-- A excecao e' nossa e e' UMA. Os botoes deles ("Update Fill Color" e
-- companhia) aplicavam um pedaco do estilo cada um, e podiam discordar entre
-- si. O `ApplyStyle` aplica o estilo inteiro pelo `SetInputValues`: quatro
-- botoes viraram um, e ele e' o unico caminho de aplicacao que existe.
local BOTOES_PERMITIDOS = { ApplyStyle = true, DisplayConfig = true }
...
for nome, inst in pairs(instancias or {}) do
	checa(inst.Page == "Text",
		nome .. ' is on the "' .. tostring(inst.Page) .. '" tab, not on "Text"')
end
