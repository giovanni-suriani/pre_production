...
-- Caminho do legendas.lua que sera' carregado.
local ARQUIVO = [[D:\CanalYtbe\BatataQuente\pre_production\projects\EpisodioPiloto\cortes\corte_pro_davinci\legendas.lua]]
...
local TEMPLATES = { "GiAutoSubs Caption", "AutoSubs Caption" }
...
local function aplicar_preset(comp, tool, preset)
	...
	if f then
		local ok, erro = pcall(function()
			loadstring(f)()(comp, tool, preset, "script", false)
		end)
...
--
-- O elemento 1 (fill) fica FORA de proposito: a cor da palavra falada nao tem
-- seletor proprio, e reusar o FillColor deixaria a palavra ativa igual as
-- outras - ou seja, mataria o destaque que ela existe pra fazer.
...
local function main(resolve, jaTentou)
	local project = resolve:GetProjectManager():GetCurrentProject()
	local mediaPool = project:GetMediaPool()

	local caminho = ARQUIVO
	if (not caminho or caminho == "") and fusion and fusion.RequestFile then
		caminho = fusion:RequestFile("", "legendas.lua",
			{ FReqB_SeqGather = false, FReqS_Title = "Pick the legendas.lua file" })
	end
	if not caminho or caminho == "" then
		log("ERROR: no input file set. Edit ARQUIVO at the top of:")
	...
	if MODO == "atualizar" then
		...
		if nomeTemplate ~= TEMPLATES[1] then
	...
	if MODO == "atualizar" then
		...
		if #existentes ~= #segmentos then
			log(string.format("      WARNING: %d captions on the timeline but %d "
				.. "in legendas.lua.", #existentes, #segmentos))
			log("      They are paired in time order, so the extra ones on the")
			log("      longer side are left alone. If the transcript changed,")
			log("      run with MODO = \"criar\" instead.")
		end
		etapa(5, 6, "no clip created (update mode)")
	else
	...
	local falhas, comDestaque, comCor, semWords = {}, {}, {}, {}
	...
	local tarefas = {}
	if MODO == "atualizar" then
		-- pareamento por ORDEM no tempo: as duas listas estao ordenadas, e o
		-- que sobrar do lado mais longo fica de fora (ja avisado acima)
		for k = 1, math.min(#existentes, #segmentos) do
			tarefas[#tarefas + 1] = { i = k, item = existentes[k].item,
				texto_atual = existentes[k].texto }
		end
	else
		for k = 1, #clipes do
			if itens[k] ~= nil then
	...
	local function estilizar(i, item, insistir, texto_atual)
		...
		viaPreset = aplicar_preset(comp, tool, preset)
		...
		-- 7) cor do falante.
		-- aplicar_em = "fill"      -> pinta o texto (elemento 1)
		-- aplicar_em = "destaque"  -> pinta a caixa da palavra falada (4)
		local sp = dados.speakers and dados.speakers[seg.speaker_id]
		local alvo_falante = nil
		if sp and sp.cor then
			alvo_falante = (sp.aplicar_em == "destaque") and 4 or 1
			for c, canal in ipairs({ "Red", "Green", "Blue" }) do
				local nome = canal .. alvo_falante
				pcall(function() tool:SetInput(nome, sp.cor[c]) end)
				if follower then
					pcall(function() follower:SetInput(nome, sp.cor[c]) end)
				end
			end
			comCor[i] = true
		end
		...
		local kf = keyframes_vazios()
		local destaque = dados.destaque
		if tempos then
			-- O keyframe escreve a cor de TODAS as palavras, entao ele
			-- sobrescreveria o SetInput do falante logo acima. A cor da pessoa
			-- precisa entrar como o estado "inativo" da camada dela - e so dela.
			if alvo_falante then
				local copia = { camadas = {} }
				for k, c in ipairs(destaque.camadas) do
					if c.elemento == alvo_falante then
						local nc = {}
						for kk, vv in pairs(c) do nc[kk] = vv end
						nc.cor_base = sp.cor
						if alvo_falante ~= 1 then nc.cor_ativa = sp.cor end
						copia.camadas[k] = nc
					else
						copia.camadas[k] = c
					end
		...
		if follower then
			...
			if tempos then comDestaque[i] = true else splineLimpo[i] = true end
		end
	...
	if MODO ~= "atualizar" then
		log(string.format("  conflict mode     %s", CONFLITO))
		if #contornados > 0 then
	...
	if MODO == "atualizar" then
		log(string.format("  captions          %d on the timeline, %d restyled, "
			.. "%d failed", #existentes, feitos, contar(falhas)))
	else
	...
	log(string.format("  speaker colour    %d clips", contar(comCor)))
