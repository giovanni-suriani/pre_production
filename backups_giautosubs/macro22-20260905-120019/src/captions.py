...
def to_giautosubs(segments, offset=0.0, casas=3):
    """Segmentos -> a lista que o GiAutoSubs reconhece.

    Nao e' um formato novo: e' a MESMA lista de `{start, end, text}` que o
    editor de turnos ja le na coluna "o que foi dito", com o `words` do Whisper
    preservado ao lado. O `words` e' o que acende a palavra falada no macro - e
    era exatamente o que se perdia quando a legenda saia so em `.srt`, que nao
    tem onde guardar tempo por palavra.

    Um arquivo, dois leitores: o `giautosubs.py <pasta> --transcript <este>` e a
    coluna do editor.

    Tempo ABSOLUTO da midia, sempre (`offset` existe para o caso de o arquivo de
    origem estar em tempo relativo ao corte). Quem desconta o inicio do corte e'
    o proprio `giautosubs.py`, lendo o `corte.json` - descontar aqui tambem
    jogaria a legenda para tras duas vezes.

    Nada de encaixar no frame aqui: o lado Lua converte segundos em frames com a
    fps da timeline. Arredondar para 3 casas e' so' para o arquivo sair igual ao
    que o `giautosubs.py` escreve, e poder ser comparado com ele.
    """
    out = []
    for seg in segments:
        text = (seg.get("text") or "").strip()
        if not text:
            continue
        item = {"start": round(float(seg["start"]) + offset, casas),
                "end": round(float(seg["end"]) + offset, casas),
                "text": text}
        palavras = seg.get("words")
        if isinstance(palavras, list) and palavras:
            item["words"] = [{"word": w["word"],
                              "start": round(float(w["start"]) + offset, casas),
                              "end": round(float(w["end"]) + offset, casas)}
                             for w in palavras
