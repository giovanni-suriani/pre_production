...
# A terceira cor da caixa. Uma "caixa de 3 cores" nao e' tres bordas
# concentricas (isso daria moldura nos quatro lados): e' a MESMA caixa desenhada
# tres vezes, deslocada. O elemento 7 ja' e' um Border Fill com cor propria e
# offset - ele e' a segunda cor, e a `box_shadow` do estilo continua sendo
# sombra normal em quem nao usa isto. Ver `_camada3`.
ELEM_CX_BASE_CAMADA3 = 8
...
def _camada3(inputs, controles, n, camada, nivel, geo, caixa_on):
    """A terceira cor da caixa do texto (elemento 8).

    So' cor e deslocamento sao dela. `nivel` e `geo` chegam prontos, os MESMOS
    que a caixa base usou - se as tres camadas divergirem em Level, Extend ou
    Round o deslocamento deixa de ser limpo e o que se ve e' um contorno torto,
    nao tres cores.
    ...
    controles["BoxLayer3Enabled"] = 1 if on else 0
    if not on:
        inputs[f"Enabled{n}"] = 0
        return False

    camada = camada or {}
    cor = camada.get("cor", "#FF4DA6")
    alpha = camada.get("alpha", 1)
    direcao = _direcao(camada) or (-0.005, 0.007)

    _elemento(inputs, n, cor=cor, nivel=nivel, offset=direcao, borda=True)
    inputs[f"Alpha{n}"] = alpha
    _geometria(inputs, n, *geo)
...
def compilar_estilo(estilo):
    ...
    caixa_base_on, _ = _caixa_e_sombra(
        inputs, controles, ELEM_CX_BASE, ELEM_CX_BASE_SOMBRA,
    ...
    # segunda fonte pros mesmos quatro numeros, e as tres camadas so' se
    # sobrepoem enquanto eles forem identicos. E' tambem o que o `camada3` do
    # macro faz: copiar do prefixo `TextBox`.
    _camada3(inputs, controles, ELEM_CX_BASE_CAMADA3,
             base.get("camada3"),
             controles.get("TextBoxLevel", 1),
             (controles.get("TextBoxExtendHorizontal", 0.2),
...
def estilo_de_controles(controles, extras=None):
    ...
    return {
        ...
        "base": {
            ...
            **({"camada3": {
                "ativo": on("BoxLayer3Enabled"),
                "cor": cor("BoxLayer3"),
                "alpha": num("BoxLayer3Alpha", 1),
                "direcao": {"x": num("BoxLayer3CenterX", -0.005),
                            "y": num("BoxLayer3CenterY", 0.007)},
            }} if any(k.startswith("BoxLayer3") for k in c) else {}),
        },
        "destaque": {
            "ativo": True,
            "cor": cor("WordFill"),
...
def para_relativo(itens, inicio):
    ...
    A decisao e' pela MAIORIA (mediana), nao pelo menor tempo do arquivo. Com o
    minimo, UM item fora da borda decidia pelo arquivo inteiro: no corte
    `rank_surpresa` do DeuMerda a primeira legenda comeca 2,3 s ANTES do corte
    (ela veio do recorte da transcricao do episodio), o `menor` caiu fora da
    folga de 1 s e a transcricao ficou em absoluto enquanto os turnos viraram
    relativos. Bases opostas = sobreposicao zero = toda legenda sem falante, e
    o unico sintoma era `None` na contagem por falante.
    """
    if not itens or inicio <= 0:
        return itens, 0.0
    comecos = sorted(float(i["start"]) for i in itens)
    meio = comecos[len(comecos) // 2]
    if meio < inicio - 1.0:
...
def _tela_dividida(pasta):
    """`split_screen` do project.json dono do corte (projects\\<p>\\cortes\\<c>).
    False quando nao ha projeto - corte solto fora do pre_production."""
    proj = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(pasta))),
                        "project.json")
    try:
        return bool(_ler_json(proj).get("split_screen"))
    except (OSError, ValueError):
        return False


# `cut` no track_map nunca foi track: significa JOGAR O TRECHO FORA (o
# SpeakerSwitch.py tem a mesma constante, pelo mesmo motivo). Aqui ele so
# precisa ficar de fora da lista de falantes - quem quiser descartar as
# legendas desses trechos usa `--pular cut`.
CUT_NAMES = {"cut"}
...
def montar(pasta, cfg, nome_estilo, usar_words=True, max_chars=None, pular=(),
    ...
    # ultimo episodio e envelhece calada - no `rank_surpresa` do DeuMerda ela
    # trazia Cupertino/Cuper/Heitor/Giovanni enquanto o corte era Heitor/Pedro:
    # o Pedro (198 dos 276 turnos) caia na track 1 por nao existir na lista, e
    # o Heitor ia pra track 2 em vez da 1. `cut` nao e' pessoa - significa
    # jogar o trecho fora - entao fica de fora da lista de falantes.
    cfg_falantes = {n: d for n, d in
                    (corte.get("track_map") or cfg.get("falantes") or {}).items()
                    if n not in CUT_NAMES}
    ...
    for nome, dados in cfg_falantes.items():
        ...
        speakers.append({
            "nome": nome,
            # o corte.json escreve o numero direto (`{"Heitor": 1}`), o
            # estilos.json embrulha num dicionario (`{"track": 1}`)
            "track": dados if isinstance(dados, (int, float))
...
def main(argv=None):
    ...
    trk.add_argument("--split-screen", dest="tela_dividida", action="store_const",
                     ...
                     help="one caption track per speaker (default: the project's "
                          "split_screen)")
    trk.add_argument("--single-track", dest="tela_dividida", action="store_const",
                     const=False,
                     help="all captions on one track (default: the project's "
                          "split_screen)")
    ...
    # os falantes DESTA rodada (o track_map do corte), nao a lista global - era
    # ela que marcava "<- base style" em nome que o corte conhece muito bem
    conhecidos = {s["nome"] for s in doc["speakers"]}
    linha = []
