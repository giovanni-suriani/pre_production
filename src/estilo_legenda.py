...
def quebrar_linhas(texto, max_chars, max_linhas=3):
    """Quebra em linhas curtas e EQUILIBRADAS. Devolve a LISTA de linhas.
    ...
    Devolve lista, e nao a string com `\\N`, porque quem chama precisa saber se
    COUBE - e uma string ja juntada nao responde isso sem desjuntar de novo. O
    `cabe()` abaixo e' o unico consumidor dessa diferenca, e ele existe pro
    giautosubs.py decidir se a legenda tem que virar duas.
    """
    ...
    # proposito - melhor uma linha longa do que legenda sumida. Quem tem o TEMPO
    # na mao (o giautosubs.py) tem uma saida melhor: partir em duas legendas.
    minimo = max(1, min(max_linhas, -(-len(texto) // max(1, max_chars))))
    melhor = None
...
def quebrar(texto, max_chars, max_linhas=3):
    """`quebrar_linhas` no formato do ASS (`\\N` entre as linhas)."""
    return "\\N".join(quebrar_linhas(texto, max_chars, max_linhas))


def cabe(texto, max_chars, max_linhas):
    """O texto cabe em `max_linhas` linhas de ate `max_chars` caracteres?

    A conta nao e' `len(texto) <= max_chars * max_linhas`: a quebra so acontece
    em espaco, entao "IMPRESSIONANTE demais" pode nao caber em duas linhas de 20
    mesmo com 21 caracteres. Quem responde e' a propria quebra.
    """
    linhas = quebrar_linhas(texto, max_chars, max_linhas)
    return len(linhas) <= max_linhas and max(len(x) for x in linhas) <= max_chars
