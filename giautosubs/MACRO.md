...
| 4 | **ControlPage** | as abas do Inspector (`Text`, `Extras`, `Style`, `Controls`, `Settings`). | `UserControls` do **macro** |
...
- **E o contrário também vale.** `Page = "Extras"` num InstanceInput só diz onde
  o controle mora; sem um `Extras = ControlPage { CT_Visible = true }` declarado
  aqui, a aba não existe e todo mundo cai **calado** na primeira página visível.
  É a mesma regra lida ao contrário — foi assim que a aba Extras nasceu (macro
  22). A ordem das declarações é a ordem das abas.
...
| `Level{n}` | `BorderLevel` | **borda** | 0-based: **0 Text, 1 Line, 2 Word, 3 Character** (corrigido no macro 22 — era `1 caractere, 2 palavra, 3 linha`, com Character e Line trocados) |
...
### A caixa de tres cores (macro 23)

Uma caixa de tres cores **nao** e' tres bordas concentricas com `Extend`
crescente — isso daria uma moldura nos quatro lados, com cada cor aparecendo em
volta de tudo. E' a **mesma caixa desenhada tres vezes, deslocada em X/Y**: por
isso uma cor aparece so' em cima e a esquerda, e a outra so' embaixo e a direita.

Consequencia boa: o mecanismo ja' existia. O elemento **7** ja' e' um Border
Fill com cor propria deslocado por `BoxShadowCenterX/Y` — ele **e'** a segunda
cor, bastando dar-lhe `Softness = 0` e opacidade 1. Nao ha nada a sacrificar; so'
a terceira cor precisava de slot, e ela e' o elemento 8.

```
6 caixa base, centrada  |  7 segunda cor, deslocada  |  8 terceira cor, deslocada
```

Tres regras, e as tres ja' custaram caro em outros elementos:

- **A geometria e' UMA so'.** `Level`, os dois `Extend` e o `Round` sao
  **copiados** do prefixo `TextBox` nos tres elementos, nunca digitados de novo.
  Divergiram, o deslocamento deixa de ser limpo e vira contorno torto — e o
  Text+ aceita isso calado. `tests\camada3.py` existe pra pegar essa divergencia.
- **Os dois offsets tem que apontar pra lados OPOSTOS.** Iguais, as duas cores
  empilham no mesmo canto e a de baixo some. Tambem coberto no teste.
- **Transparencia por `Alpha8`, nunca `Opacity8`.** O fade do AutoSubs mora nas
  `Opacity1..4` do Follower1, conectadas ao `AnimationKeyframeStretcher`; os
  elementos 5..8 **nao estao nele** (confirmado lendo o `.setting`). Isso vale
  ja' hoje pros elementos 6 e 7, entao as tres camadas se comportam igual entre
  si — o que nao fazem e' desaparecer junto com o texto. `Opacity8` fica livre
  de proposito: e' por onde o fade entraria se um dia isso incomodar.

`Enabled8` esta em `CHAVES_DO_FOLLOWER`, mas isso ja' esta resolvido: o `pin()`
do `ApplyGiStyle` escreve nos dois tools sozinho, pra todo mundo menos
`Opacity{n<=4}`.

Configuracao, nao estrutura: com `BoxLayer3Enabled` desligado o elemento 7 volta
a ser a sombra de sempre, e um estilo sem `base.camada3` nao liga o 8 — logo nao
custa nada a quem nao usa.
...
legendas.lua    ───┴─→ SetInputValues ─┬─→ SetAnimations        (só pelo botão)
   + preset .txt                       └─→ UpdateAllStyleColors  (elementos 1..3)
                                              └─→ ApplyGiStyle   (elementos 4..7)
                                                     └─→ GiRebuildHighlight (o spline)
```

**O mesmo contrato lido ao contrário é o que exporta um preset** (macro 21). O
botão `Export Config` chama `GetInputValues`, escreve `chave<TAB>valor` em
`giautosubs\presets\<nome>.txt`, e a rodada seguinte funde esse arquivo em
`dados.controles` antes de chamar `SetInputValues`. Nada disso conhece o estilo:
o `InputKeys` é o schema nas duas direções, então um controle novo entra no
preset — e volta dele — sem uma linha de código a mais. O `Generate Caption
Style` é o mesmo corpo com outro destino (o `estilos.json`, via
`gerar_macro.py --importar-estilo`).
...
MeuControle = InstanceInput {
    ...
    Page = "Text",          -- ou "Extras"; SEMPRE explícito
},
```

O **nome** do InstanceInput não precisa ser o do UserControl (`TextLabel` aponta
para `SubtitleLabel`). Quem manda é o `Source`. E **dois InstanceInputs podem
apontar para o mesmo `Source`** — é como os nove `Apply Style` de rodapé são um
botão só, em vez de nove corpos de código para divergirem entre si.

> **A `Page` daqui é decorativa.** Quem decide em que aba (e em que ordem) cada
> controle aparece é o `LAYOUT` do `gerar_macro.py`, aplicado de uma vez pelo
> `_reordenar_inputs` no fim da geração. O Inspector segue a ordem do bloco
> `Inputs` **e** um `LabelControl` engole os `LBLC_NumInputs` controles
> *seguintes* — ou seja, "em que grupo cai" e "em que aba aparece" são a mesma
> decisão, e ela mora numa lista só. Controle que não estiver no `LAYOUT` vai
> parar no fim da aba Text, **com aviso**.
...
estão coerentes, nenhum botão sobrou, as duas abas (`Text` e `Extras`) existem de
verdade e todo mundo está numa delas, cada grupo de atributos tem seu `Apply
Style` de rodapé, e os defaults batem com o estilo pedido.
...
| Mudo o **Fill Color** e a legenda continua com outra cor, diferente em cada legenda | era a cor do falante, escrita clipe a clipe e congelada no `cor_base` do `GiCamadas` — o array vence os inputs. Saiu no macro 22: o elemento 1 lê do Inspector como todos os outros |
| Desmarco **Spoken Word > Enabled** e nada muda | até o macro 21 o checkbox só trocava de ONDE a cor ativa vinha; desmarcado, ela voltava ao `cor_ativa` gravado — que é justamente a cor do destaque. Hoje ela passa a ser a mesma da base |
| Editei uma palavra no Inspector e o destaque pinta só um pedaço dela | `GiWordTiming` endereça o texto por **posição de caractere**. O `GiApplyText` realinha enquanto o número de PALAVRAS não mudar; mudou, ele avisa e só o script resolve |
| Criei um controle e ele apareceu no lugar errado / na aba errada | ele não está no `LAYOUT` (passo 3) — o gerador avisa e joga no fim da aba Text |
