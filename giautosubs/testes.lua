	...
	local mediaPool = {
		...
		AppendToTimeline = function(_, clipes)
			...
			for k = 1, #clipes do
				...
				itens[#itens + 1] = {
					-- a track PEDIDA, pro GetItemListInTrack devolver cada legenda
					-- onde ela nasceu (uma track por falante desde o TRACK_UNICA=false)
					_track = clipes[k] and clipes[k].trackIndex,
	...
	local timeline = {
		...
		GetItemListInTrack = function(_, _, track)
			...
			for k, item in ipairs(reg.na_timeline or {}) do
				-- Sem track conhecida, o item mora na do topo (o comportamento
				-- antigo do fixture). Devolver tudo na do topo com legendas em
				-- varias tracks fazia o `menorTrack` sair errado e o "substituir"
				-- parecer criar track a mais.
				if (item._track or reg.tracks) == track then
...
local function roda_falso(alvo_lua)
	...
	if d then
		...
		do
			...
			-- A opcao "None": um arquivo vazio na pasta, escolhido de
			-- proposito. Tem que valer o estilo do legendas.lua INTEIRO - e
			-- sem passar pelo `ler_preset`, senao o arquivo vazio viraria o
			-- WARNING "is this a preset from the Export Config button?"
			-- acusando o usuario do que ele acabou de pedir.
			local none_teste = aqui .. "None(teste).txt"
			local fh_n = io.open(none_teste, "w")
			checa(fh_n ~= nil, "could not write the test None file")
			if fh_n then
				fh_n:close()

				M.definir_preset(none_teste)
				local r_no, reg_no = resolve_falso(d.macro_versao)
				reg_no.com_contrato = true
				local ok_no = pcall(M.main, r_no)
				M.definir_preset(nil)
				checa(ok_no, "main blew up with the None config chosen")

				local c_no = reg_no.comps[1]
				local p_no = c_no and c_no.tool.dados._preset_recebido
				checa(p_no and p_no.BubbleRound
					== (d.controles or {}).BubbleRound,
					"None should leave the style from legendas.lua untouched "
					.. "(BubbleRound is " .. tostring(p_no and p_no.BubbleRound)
					.. ", the style says "
					.. tostring((d.controles or {}).BubbleRound) .. ")")
