...
## Macro 23 — a caixa de tres cores (05/09/2026)

O pedido: uma text box de **tres cores**. A primeira leitura foi errada e vale
registrar, porque ela custaria um redesenho inteiro: supus tres bordas
concentricas com `Extend` crescente, o que teria exigido sacrificar a sombra da
caixa por falta de slot. O usuario corrigiu olhando a referencia — o ciano so
aparece **em cima e a esquerda**, o rosa so **embaixo e a direita**. Bordas
concentricas dariam moldura nos quatro lados. O efeito e' a **mesma caixa
desenhada tres vezes, deslocada em X/Y**.

Isso muda tudo pra melhor: o mecanismo ja' existia. O elemento **7** (a Box
Shadow) ja' e' um Border Fill com cor propria e `Offset` — ele **e'** a segunda
cor, com `Softness = 0` e opacidade 1. Nada foi sacrificado; so' a terceira cor
precisou de slot, e ela e' o **elemento 8**.

```
6 caixa base, centrada  |  7 segunda cor, deslocada  |  8 terceira cor, deslocada
```

Controles novos (grupo `Box Layer 3`, aba Extras, logo abaixo do Text Box):
`BoxLayer3Enabled`, `BoxLayer3Color{Red,Green,Blue}` (grupo de cor **28/128** —
o 27 ja' e' o `Word Color` do Spoken Word), `BoxLayer3CenterX/Y` e
`BoxLayer3Alpha`. No `estilos.json`, o bloco `base.camada3`
(`ativo`/`cor`/`alpha`/`direcao`) e o estilo `caixa_tres_cores`.

Tres decisoes que sao o coracao disto:

- **A geometria e' UMA so'.** `Level`, os dois `Extend` e o `Round` sao copiados
  do prefixo `TextBox` nos tres elementos — no Lua (`camada3` do `ApplyGiStyle`)
  e no Python, onde sao **lidos de volta** dos controles que o `_caixa_e_sombra`
  acabou de escrever, em vez de recalculados. Duas fontes pros mesmos quatro
  numeros seria o padrao que este projeto ja' pagou tres vezes. Divergiram, o
  deslocamento vira contorno torto e nada reclama.
- **Os dois offsets em lados OPOSTOS.** Iguais, as cores empilham no mesmo canto
  e a de baixo some. `y` NEGATIVO desce — errei o sinal na primeira escrita do
  estilo e o teste e' o que fixa isso.
- **`Alpha8`, nunca `Opacity8`.** Confirmado lendo o `.setting`: so' as
  `Opacity1..4` do Follower1 estao conectadas ao `AnimationKeyframeStretcher`.
  Os elementos 5..8 **ja' estavam** todos fora do fade, entao a camada nova nao
  destoa das outras duas — o que as tres fazem e' nao sumir junto com o texto,
  que ja' era o comportamento do 6 e do 7. `Opacity8` fica livre pra isso.

A armadilha do `Enabled8` no `CHAVES_DO_FOLLOWER` **ja' estava resolvida**: o
`pin()` escreve nos dois tools sozinho, pra tudo menos `Opacity{n<=4}`.

Custo pra quem nao usa: **zero**. Estilo sem `camada3` nao liga o elemento 8, e
com o `BoxLayer3Enabled` desligado o elemento 7 volta a ser sombra normal — e'
configuracao por estilo, nao mudanca estrutural.

Estado: macro instalado, `MACRO OK` (38 rotinas, 0 erros), `testes.lua` ALL OK,
`tests\camada3.py` novo (7 blocos de assercao, ALL OK), `tests\quebra.py` ALL OK.
**Falta o usuario reiniciar o Resolve e recriar os clipes** — Titles sao varridos
no boot e cada clipe carrega a propria copia do macro.

Nao feito, e uma conversa outra: mais de tres cores, ou degrade. Isso muda de
mecanismo (Background + Merge dentro da comp) e nao cabe em elemento de
sombreamento.
**05/09/2026, mesmo dia — a camada 3 virou VARIANTE, a pedido do usuario.** O
`GiAutoSubs Caption` voltou a ser o macro de antes e a caixa de tres cores ganhou
Title proprio: `gerar_macro.py --camada3` gera `GiAutoSubs 3color.setting`. O
flag `COM_CAMADA3` corta em quatro lugares de uma vez — os UserControls (bloco
condicional no `controles()`), as tres listas que os publicam (`_sem_camada3()`
filtra `NOVOS`, `RODAPES` e `LAYOUT` juntos, porque o validador recusa um
controle que exista numa e falte na outra), e dois trechos Lua marcados
(`__CAMADA3_DEF__`/`__CAMADA3_CALL__` no ApplyGiStyle, `__CAMADA3_LIGA__` no
GiRebuildHighlight), recortados pelo `_recortar`, que **levanta se o marcador
sumir** em vez de gerar um macro silenciosamente incompleto — a mesma armadilha
do `str.replace` que ja custou o macro 16.

Conferido: Caption com 58 chaves / 45 controles / 9 rodapes / 11 grupos e ZERO
ocorrencias de `BoxLayer3`, `camada3()` e `LIGA[8]`; 3color com 65 / 53 / 10 / 12
e as nove InstanceInputs. Os dois `MACRO OK` (38 rotinas). Diff do Caption contra
o `.setting` do backup macro22: **duas diferencas, nenhuma da camada 3** — o
carimbo (22 -> 23) e `WordFillEnabled` 1 -> 0, que vem do `capcut_bolha` ter
mudado em 31/08 (`destaque.fill.ativo = false`); o arquivo no disco e' que estava
defasado em relacao ao `estilos.json`. Sobra um `Enabled8 = 0` gravado como input
cru do Text+, que o macro 22 nao tinha: inofensivo (elemento 8 apagado explicito)
e MANTIDO de proposito, porque num clipe que ja passou pelo 3color e' o que
garante a terceira cor apagada ao voltar pra um estilo comum.

Isso obrigou um conserto no `giautosubs.py`, e ele e' a parte que generaliza:
**estilo sem `camada3` nao pode escrever `BoxLayer3Enabled`**. Escrevia, e a
geracao do macro padrao passou a falhar com *"the style asks for it, but there is
nowhere to store it"* — o validador pegou de primeira. O mesmo vale na volta: o
`estilo_de_controles` so' emite o bloco `camada3` quando o clipe REALMENTE tem
esses controles, senao um estilo exportado de um clipe comum passaria a exigir um
controle que o macro dele nao tem. Regra: **um controle que so' existe numa
variante nao pode aparecer no estilo de quem nao a usa** - nem na ida nem na
volta.

Fio solto assumido: o carimbo e' UM numero para DUAS variantes. Um clipe do
