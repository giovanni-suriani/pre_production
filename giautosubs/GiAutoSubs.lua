...
--
-- `None(3color).txt` (qualquer nome comecando com `None`) e' a opcao "nenhuma
-- configuracao": o estilo do legendas.lua vale inteiro. Ver `eh_none`.
local PRESET_FIXO = ""
...
-- So' vale pra legendas.lua SEM a chave `track_unica`. Desde 10/09 quem decide
-- e' a caixa "Tela dividida" do projeto no pre_production: o giautosubs.py
-- grava `track_unica = not split_screen`, e o arquivo vence esta constante.
...
-- Uma mensagem de erro do Lua pode trazer o ARQUIVO INTEIRO dentro dela: o
-- `tostring(err)` carrega o trecho que o chunk estava lendo. Um `legendas.lua`
-- tem centenas de KB, e o log vira um despejo onde a causa nao se acha - foi
-- exatamente isso que aconteceu ao escolher o `testes.lua` na pergunta 3.
local function erro_curto(e)
	local s = tostring(e):gsub("%s+", " ")
...
local function ler_dados(caminho)
	...
	if not chunk then
		return nil, "this is not a valid Lua file: " .. erro_curto(err)
	end
	local ok, dados = pcall(chunk)
	if not ok then
		return nil, "this .lua file failed to run, so it is not a caption "
			.. "file: " .. erro_curto(dados)
	end
	-- Um .lua qualquer carrega e devolve nil sem erro nenhum. Sem esta
	-- checagem, escolher o arquivo errado morria mais adiante, com uma
	-- mensagem sobre um campo faltando - longe da causa, que e' simplesmente
	-- "este nao e' o arquivo".
	if type(dados) ~= "table" or type(dados.segments) ~= "table" then
		return nil, "this file is not a legendas.lua (no 'segments' in it). "
			.. "Pick the file giautosubs.py wrote, which is normally "
			.. "<cut folder>\\legendas\\legendas.lua"
	end
	return dados
end
...
-- A escolha "nenhuma configuracao", como ARQUIVO.
--
-- O dialogo que funciona da pagina Edit e' o `fusion:RequestFile`, entao a lista
-- de opcoes e' a pasta - mesmo truque do menu do `CONFLITO`. Uma opcao que nao
-- seja arquivo nao existe. Um `None*.txt` vazio na pasta e' o que da' nome ao
-- "quero o estilo do legendas.lua inteiro, sem nada por cima": antes disso o
-- unico jeito de dizer isso era CANCELAR, que e' o mesmo gesto de "errei, quero
-- sair" - e um gesto que serve pras duas coisas nao serve pra nenhuma.
--
-- Casa pelo NOME e nao pelo conteudo de proposito: o nome e' o que aparece no
-- dialogo, e o que vem depois do `None` e' recado pro usuario (`None(3color)`).
local function eh_none(caminho)
	local base = (tostring(caminho):match("([^\\/]+)$") or ""):lower()
	return base:sub(1, 4) == "none"
end
...
local function escolher_preset()
	...
	if caminho ~= "" then
		...
		-- O `None` nao conta: sozinho ele nao e' nada pra oferecer, e um
		-- dialogo cuja unica opcao e' "nenhuma" e' so' um passo a mais.
		local tem = false
		pcall(function()
			for _, item in ipairs(bmd.readdir(PRESETS_DIR .. [[\*.txt]]) or {}) do
				if not item.IsDir and not eh_none(item.Name) then tem = true end
			end
		end)
		if not tem then
	...
	-- Escolher o `None` e' uma resposta, nao uma falha: sem ele o arquivo vazio
	-- cairia no `no 'key<TAB>value' line` do `ler_preset` e o log acusaria de
	-- WARNING exatamente o que o usuario acabou de pedir.
	if eh_none(caminho) then
		if PRESET_FIXO == "" then lembrar("preset", caminho) end
		log("      'None' chosen - using the style baked into legendas.lua")
		return nil
	end
...
local function montar_keyframes(tempos, destaque, fps)
	...
	-- Sem destaque nao ha keyframe por palavra - que e' a mesma resposta de
	-- `camadas` vazio, logo abaixo.
	local camadas = destaque and destaque.camadas
...
local function main(resolve, jaTentou, respostas)
	...
	if not caminho or caminho == "" then
		...
		-- um arquivo errado vira o lugar onde o dialogo abre na proxima rodada -
		-- e ai o erro se repete sozinho, que foi como o `testes.lua` acabou
		-- sendo oferecido como padrao.
	end
	if not caminho or caminho == "" then
	...
	-- o arquivo manda (caixa "Tela dividida" do projeto); a constante e' so'
	-- o fallback de legendas.lua gerado antes dessa chave existir
	local trackUnica = TRACK_UNICA
	if dados.track_unica ~= nil then trackUnica = dados.track_unica end
	local function track_do(id)
		if cacheTrack[id] then return cacheTrack[id] end

		local sp = dados.speakers and dados.speakers[id]
		local faixa = trackUnica and 1 or ((sp and sp.track) or 1)
	...
	if trackUnica then
		log("      all captions go on a single track "
			.. (dados.track_unica ~= nil and "(project is not split screen)"
				or "(TRACK_UNICA at the top of this script)"))
	else
		log("      one track per speaker "
			.. (dados.track_unica ~= nil and "(project is split screen)"
				or "(TRACK_UNICA at the top of this script)"))
	end
