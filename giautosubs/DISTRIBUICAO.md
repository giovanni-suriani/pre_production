...
|---|---|
| `giautosubs/GiAutoSubs.lua` | `ESCOLHAS` e `PRESETS_DIR` — as duas pastas de trabalho |
...
| `src/gerar_macro.py` (`LOG_MACRO`, `PRESETS_DIR`, `ESTILO_EXPORTADO`) | caminhos absolutos colados dentro do macro |
...
- [x] **`ARQUIVO` deixou de ser caminho fixo (macro 21).** O padrão é vazio e a
      rodada PERGUNTA — junto com o Title e a configuração, são as três
      perguntas. O diálogo é `fusion:RequestFile` (o `AskUser` existe como
      atributo e vem `nil` fora da página Fusion), e o `_giescolhas.txt` guarda a
      resposta anterior só para a caixa abrir na pasta certa. Cancelar cai no
      padrão, dizendo qual foi usado. **Falta testar dentro do Resolve de
      verdade** — se o `fusion` não existir na página Edit, o script hoje para
      com uma mensagem em vez de morrer calado, mas o caminho continua sendo o
      `ARQUIVO` no topo.
...
| `ESTILO_FIXO`, `PRESET_FIXO`, `ARQUIVO` | as três perguntas da rodada — vazias, ele pergunta; preenchidas, não |
...
2. [~] `ARQUIVO` deixa de ser caminho fixo (feito no macro 21) — **falta testar
       o seletor dentro do Resolve**
