...
def _enriquecer(r: dict) -> dict:
    ...
    for j in r.get("jogos") or []:
        a = jogos.migrar(j.get("tipo"), j.get("attrs") or {})
        j["attrs"] = a
...
def _participante(x: dict) -> dict:
    """So id e nome. As vidas sairam daqui: sao de cada jogo (jogos.vidas), e
    nao da pessoa. Um `vidas_total` de um cadastro antigo e simplesmente
    largado na proxima gravacao do roteiro."""
    return {
...
def salvar_jogo(slug: str, jid: str, nome: str | None,
    ...
    if attrs is not None:
        j["attrs"] = jogos.migrar(j.get("tipo"),
                                  {**(j.get("attrs") or {}), **attrs})
    store.gravar(store.ROTEIROS, slug, r)
